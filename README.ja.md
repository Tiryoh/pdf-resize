...
ローカルで動かす場合は、`index.html` をブラウザで開くか、ローカルプレビューサーバーを起動してアクセスします。
...
2. 圧縮プリセットを選択、または詳細設定を調整
3. 「圧縮開始」をクリック
4. 処理完了後「ダウンロード」で圧縮済みPDFを取得

### 圧縮プリセット

| プリセット | JPEG品質 | 最大長辺 | 説明 |
|---|---|---|---|
| 軽量 | 90% | 4096px | 高品質・最小圧縮 |
| 標準 | 75% | 2048px | バランス重視（デフォルト） |
| 最大圧縮 | 50% | 1024px | サイズ優先 |
...
| モード | 説明 |
|---|---|
| 個別圧縮（デフォルト） | RGBとアルファマスク（SMask）を別々にJPEG圧縮 |
| 白背景で合成 | 白背景に合成してから圧縮 |
| スキップ | 透過画像はそのまま保持 |
